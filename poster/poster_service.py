...
from poster.image_processor import analyze_image, crop_to_size, generate_placeholder_background
...
def generate_poster(request: PosterRequest, *, business_id: str = "", regeneration_seed: str | None = None,
                     demo_mode: bool = False) -> GenerationResult:
    ...
    demo_mode=True skips the paid AI call entirely — a Pillow-generated
    placeholder background stands in for the artwork, run through the same
    real typography/logo pipeline, clearly watermarked. Lets the whole UI
    and the deterministic-text output be exercised for free.
    """
    ...
    try:
        ...
        if demo_mode:
            spec = POSTER_SIZES[request.output_size_key]
            normalized = generate_placeholder_background(spec["width"], spec["height"], request.style_key)
        else:
            image_analysis = None
        ...
        metadata = GenerationMetadata(
            ...
            provider="demo" if demo_mode else "openai",
            model="placeholder" if demo_mode else POSTER_IMAGE_MODEL_DEFAULT,
            prompt_version=POSTER_PROMPT_VERSION,
            created_at=datetime.now(dt_timezone.utc).isoformat(),
            duration_seconds=duration,
            success=True,
        )
        print(f"poster_studio: generation {generation_id} succeeded in {duration}s (category={category.key}, demo_mode={demo_mode})")
